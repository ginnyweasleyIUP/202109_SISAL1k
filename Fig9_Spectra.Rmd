...
Mask_hadcm3 is introduced because some spectra calculation in hadcm3 must have gone wrong somehow. The reasons will be looked for soon. We excluded all entities where the spectra produced a 3 times higher variance than the variance calculation: 

```{r}
mask_hadcm3= c(T,T,T,T,T,F,F,F,F,F,F,F,F,T,T,T,F,F,F,F,T,F,F,F,F,F,F,F,T,T,T,T,T,F,F,T,F,F,F,F,F,F,F,F,F,F,T,F,F,F,F,F,T,F,F,F,F,T,F,F,F,T,F,F,F,T,F,T,F,F,F,F,F,T,F,F,F,T,F,F,F,F,F,T,T,F,T,F)
mask_hadcm3 = rep(F,length(mask_hadcm3))
#mask_hadcm3[53] = T #that's entity_id 348 which is weird...
#mask_hadcm3 = c(F, T, F, F, F, F, F, F, T, T, F, F, T, F, F, F, F, F, F, F, T, T, T, T, F, F, F, F, F, F, F, F, F, T, F, F, T, F, F, F, F, F, T, F, F, F, F, F, T, F, F, F, F, F, T, F, F)
#mask_hadcm3 = c(F,F,F,F,
                # T, F, F, F, F,F,
                # F, F, F, T, T, F,F,F,
                # F, F, T, F, F, F,
                # F, F, F, F, F, T, T, T, T, F, F, F, F, F, F, F, F, F, F,F,F,F,
                # T, F,
                # F, F,F,F, 
                # T,F,
                # F, F, F, F,
...
for(Model in c("HadCM3", "ECHAM5", "GISS", "CESM", "isoGSM")){
  print(Model)
  for(var in c("ISOT", "ITPC", "ITIF", "ITSL")){
    ...
    for(entity in ENTITY_INFO$entity_id){
      ...
      if(Model == "HadCM3" & mask_hadcm3[ENTITY_INFO$entity_id == entity]){next}
      if(Model %in% c("CESM", "isoGSM") & entity == 378 & var == "ITIF"){next}
      if(Model %in% c("ECHAM5") & entity %in% c(672, 598) & var == "ITIF"){next}
      ...
      #Down spectra
      if(sum(is.na(data_down[[var]]))>length(data_down[[var]])/4*3){
        #print(entity)
        next
        }
...
cairo_pdf(file = paste0("Plots/Manuscript_7_Spectra.pdf"), width = 10/2*3, height = 6)
#svg(file = paste0("Plots/Manuscript_7_Spectra.svg"), width = 10/2*3, height = 6)


#COLZ <- c("#1A2254", "#0A4296", "#278BCE","#91002B", "#D6604D", "black")
#IPCC
#COLZ = c("red", rgb(0,0,0), rgb(112,160,205,max = 255), rgb(196,121,0,max = 255), rgb(178,178,178,max = 255), rgb(0,52,102,max = 255))
COLZ = c("red", rev(RColorBrewer::brewer.pal(6, "YlGnBu")[2:6]))
#COLZ = c(rgb(112,160,205, max = 255), rgb(196,121,0, max = 255),rgb(178,178,178, max = 255),rgb(0,52,102, max = 255),rgb(0,121,0, max = 255))
COLZ = c(rgb(112,160,205, max = 255), rgb(0,52,102, max = 255), rgb(178,178,178, max = 255),rgb(0,121,0, max = 255), rgb(196,121,0, max = 255))

COLZ_carbon = "black"#rgb(33, 102, 172, max = 255)
...
mtext("a) speleo-d18O/d13C", side = 3, line = -2, adj = 0.05, cex = cex_text)


#b) second plot is spec ratio between speleo-d18O and downsampled d18O
...
mtext("Period (y)", side = 1, line= 2, cex = cex_axis_text)
#mtext(expression("PSD [‰"^"2"*"year]"), side = 4, line= 2.5, at = 1, las = 0, cex = cex_axis_text)
...
legend("bottomleft", legend = c("ECHAM5-wiso","GISS", "iCESM","iHadCM3", "isoGSM"),
       text.col = COLZ, 
       #lwd = rep(lw_smooth,5), lty = rep(1,5), 
       bty = "n", cex = cex_text_legend)

mtext(" b) speleo-d18O / modeled d18O (record res.)", side = 3, line = -2, adj = 0.05, cex = cex_text)


#c) second plot is spec ratio between speleo-d18O and annual d18O
LPlot(spec_ratio, col = adjustcolor("white", alpha = 0),
      ylim = c(0.0002,100), xlim = c(1/300, 0.5),
      ylab = "",
      xaxt = 'n',
...
# legend("bottomleft", legend = c("HadCM3", "ECHAM5", "GISS", "CESM", "isoGSM"),
#        text.col = COLZ, 
#        #lwd = rep(lw_smooth,5), lty = rep(1,5), 
#        bty = "n", cex = cex_text_legend)

mtext(" c) speleo-d18O / modeled d18O (annual res.)", side = 3, line = -2, adj = 0.05, cex = cex_text)
...
Calculating slopes for the spectra and comparing them to specific climate variables did not bring further insight. We saw no relationship whatsoever between all the tested simulated variables (temperature, precipitation, evaporation), also not when separated into specific climatic regimes. More details in 4_Spectra.Rmd
...
Let's try histograms of the variance. This is also variability but on the complete timescale. 

isoGSM: the variance is much higher than the varfromspec variance --> so sth in the spectrum calculation goes wrong
        This is for both downsampled and full
same for ECHAM5, GISS, CESM
HadCM3: here we have alterations into both directions. So both higher and lower

The change to calculating 300y variances instead of 1000y variances doesn't make a huge difference...
...
VAR_FIELDS<-list()
load("Data/Annual_VarFields/Annual_HadCM3_ISOT.RData")
VAR_FIELDS$HadCM3 <- apply(ANNUAL[,,], c(1,2), var, na.rm = T)
VAR_FIELDS$HadCM3_300 <- apply(ANNUAL[,,200:500], c(1,2), var, na.rm = T)
load("Data/Annual_VarFields/Annual_ECHAM5_ISOT.RData")
VAR_FIELDS$ECHAM5 <- apply(ANNUAL[,,], c(1,2), var, na.rm = T)
VAR_FIELDS$ECHAM5_300 <- apply(ANNUAL[,,200:500], c(1,2), var, na.rm = T)
load("Data/Annual_VarFields/Annual_GISS_ISOT.RData")
VAR_FIELDS$GISS <- apply(ANNUAL[,,], c(1,2), var, na.rm = T)
VAR_FIELDS$GISS_300 <- apply(ANNUAL[,,200:500], c(1,2), var, na.rm = T)
load("Data/Annual_VarFields/Annual_CESM_ISOT.RData")
VAR_FIELDS$CESM <- apply(ANNUAL[,,], c(1,2), var, na.rm = T)
VAR_FIELDS$CESM_300 <- apply(ANNUAL[,,200:500], c(1,2), var, na.rm = T)
load("Data/Annual_VarFields/Annual_isoGSM_ISOT.RData")
VAR_FIELDS$isoGSM <- apply(ANNUAL[,,], c(1,2), var, na.rm = T)
VAR_FIELDS$isoGSM_300 <- apply(ANNUAL[,,200:500], c(1,2), var, na.rm = T)
...
for(Model in c("HadCM3", "ECHAM5", "GISS", "CESM", "isoGSM")){
  ...
  for(entity in ENTITY_INFO$entity_id){
    ...
    if(Model %in% c("GISS", "CESM")){variable = "ITPC"}
    else{variable = "ITIF"}
    
    f_start = FirstElement(SPECTRA$FULL_D[[Model]][[variable]][[paste0("ENTITY", entity)]]$freq)
    f_end = LastElement(SPECTRA$FULL_D[[Model]][[variable]][[paste0("ENTITY", entity)]]$freq)
    if(f_start<1/300){f_start = 1/300}
    if(f_start>0.5){f_end = 0.5}
    
    VARIANCE[[Model]]$FULL = c(VARIANCE[[Model]]$FULL, var(data_full[[variable]], na.rm = T))
    VARIANCE[[Model]]$FULL_300 = c(VARIANCE[[Model]]$FULL_300, var(data_full[[variable]][200:500], na.rm = T))
    VARIANCE[[Model]]$DOWN = c(VARIANCE[[Model]]$DOWN, var(data_rec[[variable]], na.rm = T))
    VARIANCE[[Model]]$REC = c(VARIANCE[[Model]]$REC, var(data_rec$d18O_measurement, na.rm = T))
    
    VARIANCE[[Model]]$FULL_SPEC = c(VARIANCE[[Model]]$FULL_SPEC, 
                                    PaleoSpec::GetVarFromSpectra(SPECTRA$FULL_D[[Model]][[variable]][[paste0("ENTITY", entity)]], f = c(f_start, f_end))$var)
    
    if(!is.null(SPECTRA$DOWN_D[[Model]][[variable]][[paste0("ENTITY", entity)]])){
      f_start = FirstElement(SPECTRA$DOWN_D[[Model]][[variable]][[paste0("ENTITY", entity)]]$freq)
      f_end = LastElement(SPECTRA$DOWN_D[[Model]][[variable]][[paste0("ENTITY", entity)]]$freq)
      if(f_start<1/300){f_start = 1/300}
      if(f_start>0.5){f_end = 0.5}
      VARIANCE[[Model]]$DOWN_SPEC = c(VARIANCE[[Model]]$DOWN_SPEC, 
                                    PaleoSpec::GetVarFromSpectra(SPECTRA$DOWN_D[[Model]][[variable]][[paste0("ENTITY", entity)]], f = c(f_start, f_end))$var)
      VARIANCE[[Model]]$REC_SPEC = c(VARIANCE[[Model]]$REC_SPEC, 
                                   PaleoSpec::GetVarFromSpectra(SPECTRA$RECD_D$d18O[[paste0("ENTITY", entity)]], f = c(f_start, f_end))$var)
    }else{
      VARIANCE[[Model]]$DOWN_SPEC = c(VARIANCE[[Model]]$DOWN_SPEC,NA)
      VARIANCE[[Model]]$REC_SPEC = c(VARIANCE[[Model]]$REC_SPEC, NA)
    }
  ...
  VARIANCE[[Model]]$FULL_300 = as.numeric(VARIANCE[[Model]]$FULL_300)
  VARIANCE[[Model]]$FULL_SPEC = as.numeric(VARIANCE[[Model]]$FULL_SPEC)
  VARIANCE[[Model]]$DOWN = as.numeric(VARIANCE[[Model]]$DOWN)
  VARIANCE[[Model]]$DOWN_SPEC = as.numeric(VARIANCE[[Model]]$DOWN_SPEC)
  VARIANCE[[Model]]$REC = as.numeric(VARIANCE[[Model]]$REC)
  VARIANCE[[Model]]$REC_SPEC = as.numeric(VARIANCE[[Model]]$REC_SPEC)
  VARIANCE[[Model]]$entity_id = as.numeric(VARIANCE[[Model]]$entity_id)
  
  # VARIANCE[[Model]]$FULL[VARIANCE[[Model]]$FULL>3] = NA
  # VARIANCE[[Model]]$FULL_300[VARIANCE[[Model]]$FULL_300>3] = NA
  # VARIANCE[[Model]]$DOWN[VARIANCE[[Model]]$DOWN>3] = NA
  # VARIANCE[[Model]]$REC[VARIANCE[[Model]]$REC>3] = NA
}
...
for(Model in c("ECHAM5", "GISS", "CESM", "HadCM3", "isoGSM")){
  ...
  print(paste(Model," 300y :", round(simpleawmean(VAR_FIELDS[[paste0(Model,"_300")]], 
                                            lats = seq(-90,90,length.out = dim(VAR_FIELDS[[paste0(Model,"_300")]])[2])), digits = No.digits),
              "90% CI: (",round(quantile(as.numeric(VAR_FIELDS[[paste0(Model,"_300")]]), probs = 0.05, na.rm = T),digits = No.digits),",",
              round(quantile(as.numeric(VAR_FIELDS[[paste0(Model,"_300")]]), probs = 0.95, na.rm = T),digits = No.digits),")"))
  exp_var_global = c(exp_var_global, paste0("& ",round(simpleawmean(VAR_FIELDS[[paste0(Model,"_300")]], 
                                                                    lats = seq(-90,90,length.out = dim(VAR_FIELDS[[paste0(Model,"_300")]])[2])), 
                                                       digits = No.digits)," (",
                                            round(quantile(as.numeric(VAR_FIELDS[[paste0(Model,"_300")]]), probs = 0.05, na.rm = T),digits = No.digits), ", ",
                                            round(quantile(as.numeric(VAR_FIELDS[[paste0(Model,"_300")]]), probs = 0.95, na.rm = T),digits = No.digits),")"))
  ...
  print(paste("Annual resolution at speleo location 300y",Model,":", round(median(as.numeric(VARIANCE[[Model]]$FULL_300), na.rm = T), digits = No.digits),
            "90% CI: (",round(quantile(as.numeric(VARIANCE[[Model]]$FULL_300), probs = 0.05, na.rm = T), digits = No.digits),
            ",",round(quantile(as.numeric(VARIANCE[[Model]]$FULL_300), probs = 0.95, na.rm = T), digits = No.digits),")"))
  print(paste("Annual resolution at speleo location SPEC",Model,":", round(median(as.numeric(VARIANCE[[Model]]$FULL_SPEC), na.rm = T), digits = No.digits),
            "90% CI: (",round(quantile(as.numeric(VARIANCE[[Model]]$FULL_SPEC), probs = 0.05, na.rm = T), digits = No.digits),
            ",",round(quantile(as.numeric(VARIANCE[[Model]]$FULL_SPEC), probs = 0.95, na.rm = T), digits = No.digits),")"))
  
  exp_var_speleo_full = c(exp_var_speleo_full, paste0("& ", round(median(as.numeric(VARIANCE[[Model]]$FULL_300), na.rm = T), digits = No.digits), " (",
                                                      round(quantile(as.numeric(VARIANCE[[Model]]$FULL_300), probs = 0.05, na.rm = T), digits = No.digits),", ",
                                                      round(quantile(as.numeric(VARIANCE[[Model]]$FULL_300), probs = 0.95, na.rm = T), digits = No.digits),")"))
  ...
  print(paste("Record resolution at speleo location SPEC",Model,":", round(median(as.numeric(VARIANCE[[Model]]$DOWN_SPEC), na.rm = T), digits = No.digits),
            "90% CI: (",round(quantile(as.numeric(VARIANCE[[Model]]$DOWN_SPEC), probs = 0.05, na.rm = T), digits = No.digits),
            ",",round(quantile(as.numeric(VARIANCE[[Model]]$DOWN_SPEC), probs = 0.95, na.rm = T), digits = No.digits),")"))
...
#HadCM3
#lines(density(VAR_FIELDS$HadCM3, na.rm = T), lwd = 2, col = RColorBrewer::brewer.pal(6, "YlGnBu")[6])
#lines(density(VAR_FIELDS$ECHAM5, na.rm = T), lwd = 2, col = RColorBrewer::brewer.pal(6, "YlGnBu")[5])
#lines(density(VAR_FIELDS$GISS, na.rm = T), lwd = 2, col = RColorBrewer::brewer.pal(6, "YlGnBu")[4])
#lines(density(VAR_FIELDS$CESM, na.rm = T), lwd = 2, col = RColorBrewer::brewer.pal(6, "YlGnBu")[3])
#lines(density(VAR_FIELDS$isoGSM, na.rm = T), lwd = 2, col = RColorBrewer::brewer.pal(6, "YlGnBu")[2])
...
cairo_pdf(file = paste0("Plots/Manuscript_7_Spectra_noratio.pdf"), width = 10/2*3, height = 6)
...
LLines(spec_ratio, col = adjustcolor("black", alpha.f = 0.3), lty = 1, lw = 2)
LLines(LogSmooth(spec_ratio), col = "black", lw = lw_smooth)
LLines(SPECTRA$RECD$d13C$Mean$spec, col = adjustcolor("red", alpha.f = 0.3))
LLines(LogSmooth(SPECTRA$RECD$d13C$Mean$spec), col = "red", lw = lw_smooth)
...
for(ii in 1:5){
  ...
  spec_ratio <- SPECTRA$RECD$d18O$Mean$spec
  spec_ratio$spec <- SPECTRA$RECD$d18O$Mean$spec$spec/SPECTRA$DOWN[[models[ii]]][[variable]]$Mean$spec$spec
  spec_ratio$lim.1 <- SPECTRA$RECD$d18O$Mean$spec$lim.1/SPECTRA$DOWN[[models[ii]]][[variable]]$Mean$spec$lim.1
  spec_ratio$lim.2 <- SPECTRA$RECD$d18O$Mean$spec$lim.2/SPECTRA$DOWN[[models[ii]]][[variable]]$Mean$spec$lim.2
  LLines(SPECTRA$DOWN[[models[ii]]]$ISOT$Mean$spec, col = adjustcolor(COLZ[ii], alpha.f = 0.2), lty = 1, lw = 2)
}
for(ii in 1:5){
  if(ii %in% c(2,3)){variable = "ITPC"}
  else{variable = "ITIF"}
  ...
  spec_ratio$lim.1 <- SPECTRA$RECD$d18O$Mean$spec$lim.1/SPECTRA$DOWN[[models[ii]]][[variable]]$Mean$spec$lim.1
  spec_ratio$lim.2 <- SPECTRA$RECD$d18O$Mean$spec$lim.2/SPECTRA$DOWN[[models[ii]]][[variable]]$Mean$spec$lim.2
  LLines(LogSmooth(SPECTRA$DOWN[[models[ii]]]$ISOT$Mean$spec), col = COLZ[ii], lty = 1, lw = lw_smooth)
}
...
for(ii in 1:5){
  ...
  LLines(SPECTRA$FULL[[models[ii]]]$ISOT$Mean$spec, col = adjustcolor(COLZ[ii], alpha.f = 0.2), lty = 1, lw = 2)
}
for(ii in 1:5){
  if(ii %in% c(2,3)){variable = "ITPC"}
  else{variable = "ITIF"}
  ...
  spec_ratio$lim.1 <- SPECTRA$RECD$d18O$Mean$spec$lim.1/SPECTRA$FULL[[models[ii]]][[variable]]$Mean$spec$lim.1
  spec_ratio$lim.2 <- SPECTRA$RECD$d18O$Mean$spec$lim.2/SPECTRA$FULL[[models[ii]]][[variable]]$Mean$spec$lim.2
  LLines(LogSmooth(SPECTRA$FULL[[models[ii]]]$ISOT$Mean$spec), col = COLZ[ii], lty = 1, lw = lw_smooth)
}
...
cairo_pdf(file = paste0("Plots/Supplements/Manuscript_7_Spectra_Filter.pdf"), width = 10/2*3, height = 6)
#svg(file = paste0("Plots/Manuscript_7_Spectra.svg"), width = 10/2*3, height = 6)
...
mtext(" b) speleo-d18O / modeled d18O (record res.)", side = 3, line = -2, adj = 0.05, cex = cex_text)
mtext(" FILTERED 2.5y", side = 3, line = -4, adj = 0.05, cex = cex_text)
...
models = c("ECHAM5","GISS", "CESM","HadCM3", "isoGSM")
for(ii in 1:5){
  if(ii %in% c(1,2,3)){variable = "ITPC"}
  else{variable = "ITIF"}
  #variable = "ISOT"
  spec_ratio <- SPECTRA$RECD$d18O$Mean$spec
  spec_ratio$spec <- SPECTRA$RECD$d18O$Mean$spec$spec/SPECTRA$FULL[[models[ii]]][[variable]]$Mean$spec$spec
