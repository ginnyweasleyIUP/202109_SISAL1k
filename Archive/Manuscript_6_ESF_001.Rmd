...
DATA_volc$value_CRO <- volcanic_forcing_yearly[50:1049]
#data_volc$value <- as.numeric(volcanic_forcing)[1801:37801]
#data_volc$time <- -1*index(volcanic_forcing)[1801:37801]
Gao_volcano = read.delim("Data/Gao_volc_501-2000.txt", sep = " ", stringsAsFactor = FALSE)
DATA_volc$value_GAO = apply(cbind(Gao_volcano$X.6, Gao_volcano$X.7, Gao_volcano$SH),1,sum,na.rm = TRUE)[1:1000]/150


rm(volcanic_forcing, volcanic_forcing_yearly, Gao_volcano)
...
save(SYNCHRO_SIG, file = "SYNCHRO_SIG_2000.RData")
...
pdf(file = paste0("Plots/Manuscript_6_ESF.pdf"), width = 8, height = 5)
...
Find mean time uncertainty over samples:
```{r}
sample = read.csv("~/Dokumente/01_Promotion/06_Daten/02_SISAL/SISAL_v2/sample.csv", stringsAsFactors=FALSE) %>% filter(entity_id %in% ENTITY_INFO$entity_id)
original_chrono = read.csv("~/Dokumente/01_Promotion/06_Daten/02_SISAL/SISAL_v2/original_chronology.csv", stringsAsFactors=FALSE) %>% filter(sample_id %in% sample$sample_id) %>% filter(interp_age>100 & interp_age<1100)
sisal_chrono = read.csv("~/Dokumente/01_Promotion/06_Daten/02_SISAL/SISAL_v2/sisal_chronology.csv", stringsAsFactors=FALSE) %>% filter(sample_id %in% original_chrono$sample_id)
for(ii in 2:22){
  sisal_chrono[[ii]] = as.numeric(sisal_chrono[[ii]])  
}
```
```{r message=FALSE, warning=FALSE}
UNCERT = list(lin_interp = c(), lin_reg = c(), Bchron = c(), Bacon = c(), oxcal = c(), copra = c(), StalAge = c(), original = c())
RES = list(lin_interp = c(), lin_reg = c(), Bchron = c(), Bacon = c(), oxcal = c(), copra = c(), StalAge = c(), original = c())
for(entity in ENTITY_INFO$entity_id){
  sample_entity = sample %>% filter(entity_id == entity)
  sisal_entity = sisal_chrono %>% filter(sample_id %in% sample_entity$sample_id)
  original_entity = original_chrono %>% filter(sample_id %in% sample_entity$sample_id)
  for(ii in 1:7){
    uncerts = c(sisal_entity[[(3+(ii-1)*3)]],sisal_entity[[(4+(ii-1)*3)]])
    if(all(is.na(uncerts))){UNCERT[[ii]] = c(UNCERT[[ii]],NA)}
    else{
      UNCERT[[ii]] = c(UNCERT[[ii]],mean(uncerts, na.rm = T))
      RES[[ii]] = c(RES[[ii]], mean(diff(sisal_entity[[(2+(ii-1)*3)]]), na.rm = T))
      }
  }
  uncerts = c(as.numeric(original_entity$interp_age_uncert_pos),as.numeric(original_entity$interp_age_uncert_neg))
  if(all(is.na(uncerts))){UNCERT[[8]] = c(UNCERT[[8]],NA)}
  else{
    UNCERT[[8]] = c(UNCERT[[8]],mean(uncerts, na.rm = T))
    RES[[8]] = c(RES[[8]], mean(diff(original_entity$interp_age), na.rm = T))
  }
}
for(ii in 1:8){
  UNCERT[[ii]] = as.numeric(UNCERT[[ii]])
  RES[[ii]] = as.numeric(RES[[ii]])
}

print(paste0("SISAL Median Uncertainty: ", round(median(c(UNCERT[[1]],UNCERT[[2]],UNCERT[[3]],UNCERT[[4]],UNCERT[[5]],UNCERT[[6]],UNCERT[[7]]), na.rm = T), digits = 3)))
print(paste0("SISAL Median resolution: ", round(median(c(RES[[1]],RES[[2]],RES[[3]],RES[[4]],RES[[5]],RES[[6]],RES[[7]]), na.rm = T), digits = 3)))

print(paste0("Original Median Uncertainty: ", round(median(c(UNCERT[[8]]), na.rm = T), digits = 3)))
print(paste0("Original Median resolution: ", round(median(c(RES[[8]]), na.rm = T), digits = 3)))

bstrap <- c()
for (i in 1:1000){
  bstrap <- c(bstrap,median(sample(RES[[8]],length(RES[[8]]),replace=T), na.rm = T))
}

print(paste0("Original Median resolution: ", round(median(RES[[8]], na.rm = T),digits = 2), "[‰^2],",
                 " 90% CI: (", round(quantile(bstrap,0.05), digits = 2), ", ", round(quantile(bstrap,0.95), digits = 2), ")"))


```

Check for the records that show event synchronicity with the volcanoes. It may be the ones that also show high correlation with temperature? 
Also wondering if it's the one's with that are also high resolved
...
ESF_GAO <- c()
ESF_temp <- c()
models = c("HadCM3", "ECHAM5", "GISS", "isoGSM", "CESM")
for(entity in ENTITY_INFO$entity_id){
  for(ii in 1:5){
    data_rec = DOWN[[models[ii]]] %>% filter(entity_id == entity)
    data_full = FULL[[models[ii]]] %>% filter(entity_id == entity)
    resolution = c(resolution, mean(diff(data_rec$year_BP), na.rm = T))
    CORR = cor.test(data_rec$d18O_measurement, data_rec$TEMP)
    corr_temp = c(corr_temp, CORR$estimate[[1]])
    p_value = c(p_value, CORR$p.value)
    if(ii %in% c(1,2,3)){
      corr_volc = c(corr_volc, cor.test(data_full$TEMP, DATA_volc$value_CRO)$estimate[[1]])
      p_value_volc = c(p_value_volc, cor.test(data_full$TEMP, DATA_volc$value_CRO)$p.value)
      ESF_temp = c(ESF_temp, ES.strength(TS_1 = list(time = data_rec$year_BP, value = data_rec$TEMP),
                                  TS_2 = list(time = DATA_volc$time, value = DATA_volc$value_CRO),q = 5, cf = F, volc = T)$strength)
    }else{
      corr_volc = c(corr_volc, cor.test(data_full$TEMP, DATA_volc$value_GAO)$estimate[[1]])
      p_value_volc = c(p_value_volc, cor.test(data_full$TEMP, DATA_volc$value_GAO)$p.value)
      ESF_temp = c(ESF_temp, ES.strength(TS_1 = list(time = data_rec$year_BP, value = data_rec$TEMP),
                                  TS_2 = list(time = DATA_volc$time, value = DATA_volc$value_GAO),q = 5, cf = F, volc = T)$strength)
    }
  }
  
  if(!all(is.na(data_rec$d13C_measurement))){
    corr_isotopes = c(corr_isotopes, cor.test(data_rec$d18O_measurement, data_rec$d13C_measurement)$estimate[[1]])
    ESF_isotopes = c(ESF_isotopes, ES.strength(TS_1 = list(time = data_rec$year_BP, value = data_rec$d18O_measurement),
                                               TS_2 = list(time = data_rec$year_BP, value = data_rec$d13C_measurement),q = 5, cf = F)$strength)
  }
  
  
  
  ESF_GAO = c(ESF_GAO,ES.strength(TS_1 = list(time = data_rec$year_BP, value = data_rec$d18O_measurement),
                                  TS_2 = list(time = DATA_volc$time, value = DATA_volc$value_GAO),q = 5, cf = F, volc = T)$strength)
    ESF_CRO = c(ESF_CRO,ES.strength(TS_1 = list(time = data_rec$year_BP, value = data_rec$d18O_measurement),
                                    TS_2 = list(time = DATA_volc$time, value = DATA_volc$value_CRO),q = 5, cf = F, volc = T)$strength)
}

corr_temp[p_value>0.1] = NA
#calculate mean over consecutive 5
resolution_mean = zoo::rollapply(resolution,5,mean, by = 5, na.rm = T)
corr_temp_mean = zoo::rollapply(corr_temp,5,mean, by = 5, na.rm = T)
#calculate how much agree
```

While for the correlation, it seems as if the correlation increases with coarser resolution. For the event synchronization it is the other way around: the lower the resolution, the higher the ES-Strength. 
Careful: this is not contradictory - it's the correlation to simulated temperature compared to the ESF with volcanoes. ESF and correlation to the same variables still correlate! 

Just double checking: ESF of volcanoes and simulated temperature: 
```{r}
corr_volc[p_value_volc>0.1] = NA
plot(resolution_mean, abs(corr_temp_mean))
plot(resolution_mean, ESF_GAO)
plot(resolution_mean, ESF_CRO)
plot(abs(corr_volc), ESF_temp, col = "red", main = "simlated Temperature and Volcanoes")
plot(abs(corr_isotopes), ESF_isotopes)
```
