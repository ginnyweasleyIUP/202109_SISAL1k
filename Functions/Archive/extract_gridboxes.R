...
extract_gridboxes <- function(lon_cave, lat_cave, d.lon = 3.75, d.lat = 2.5, which_model){
  result_list <- list()
  #lon_list <- seq(from = 0, to = 360-d.lon, by = d.lon)
  #lat_list <- seq(from = 90, to = -90, by = -d.lat)
  lon_list = DATA_past1000[[which_model]]$lon
  lat_list = DATA_past1000[[which_model]]$lat
