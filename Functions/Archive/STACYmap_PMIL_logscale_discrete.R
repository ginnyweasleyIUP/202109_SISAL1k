...
STACYmap_isot_discrete <- function(gridlyr = NULL, 
                     ptlyr = NULL, 
                     ptlyr_size_var = NULL,
  ...
  if (!is.null(gridlyr)) {
    ...
    map_plot <- map_plot + scale_fill_stepsn(colors = {if (splcol) {colorscheme$grid} else {colorscheme}},
                                             limits = lims$grid, na.value = "grey50",
                                             n.breaks = discrete_breaks,nice.breaks = T,labels = labels_discrete,
                                             guide = clrgd(legend_names$grid,
                                                           breaks = signif(seq(lims$grid[1], lims$grid[2], length.out = discrete_breaks), digits = 2))) +
        scale_color_stepsn(colors = {if (splcol) {colorscheme$grid} else {colorscheme}}, 
                           limits = lims$grid, n.breaks = discrete_breaks, nice.breaks = T, guide = FALSE)
