...
isot_lyr_HadCM3 <- MEAN_FIELD$PREC
isot_GM_HadCM3 <- simpleawmean(apply(MEAN_FIELD$PREC,c(1,2),mean,na.rm=TRUE))
#ECHAM5
load("Data/SISAL1k_MeanField_ECHAM5.RData")
isot_lyr_ECHAM5 <- MEAN_FIELD$PREC
isot_GM_ECHAM5 <- simpleawmean(apply(MEAN_FIELD$PREC,c(1,2),mean,na.rm=TRUE), lats = seq(from = -90,to = 90,length.out = 48))
#CESM
load("Data/SISAL1k_MeanField_CESM.RData")
isot_lyr_CESM <- MEAN_FIELD$PREC
isot_GM_CESM <- simpleawmean(apply(MEAN_FIELD$PREC,c(1,2),mean,na.rm=TRUE), lats = seq(from = -90,to = 90,length.out = 96))
#GISS
load("Data/SISAL1k_MeanField_GISS.RData")
isot_lyr_GISS <- MEAN_FIELD$PREC
isot_GM_GISS <- simpleawmean(apply(MEAN_FIELD$PREC,c(1,2),mean,na.rm=TRUE), lats = seq(from = -90,to = 90,length.out = 90))
#isoGSM
load("Data/SISAL1k_MeanField_isoGSM.RData")
isot_lyr_isoGSM <- MEAN_FIELD$PREC
isot_GM_isoGSM <- simpleawmean(apply(MEAN_FIELD$PREC,c(1,2),mean,na.rm=TRUE), lats = seq(from = -90,to = 90,length.out = 94))
rm(MEAN_FIELD)
```
...
plot_HadCM3 <- STACYmap_discrete(rbind((isot_lyr_HadCM3)[49:96,],(isot_lyr_HadCM3)[1:48,]),
                                 legend_name = list(grid = "P (mm/year)"), graticules = T, colorscheme = "prcp_grd", 
                                 limits_color = list("max" = 10000, "min" = 0),
                                 labels_discrete = c(0,"",2000,"",4000,"",6000,"",8000,"",10000), 
                                 discrete_breaks = seq(0,10000, length.out = 11)) +
...
plot_isoGSM <- STACYmap_discrete(rbind(isot_lyr_isoGSM[97:192,1:94],isot_lyr_isoGSM[1:96,1:94]),
                                 ...
                                 discrete_breaks = seq(0,10000, length.out = 11)) +
  theme(panel.border = element_blank(), legend.background = element_blank(), axis.text = element_blank(), legend.text = element_text(size = 10),plot.title = element_text(hjust = 0.5)) +
  ggtitle(paste0("iCESM, GM: ",round(simpleawmean(isot_lyr_CESM, lats = seq(from = -90,to = 90,length.out = 96)), digits = 1), "mm/year"))
...
plot_CESM <- STACYmap_discrete(rbind(isot_lyr_CESM[73:144,1:96],isot_lyr_CESM[1:72,1:96]),
                               legend_name = list(grid = "P (mm/year)"), graticules = T, colorscheme = "prcp_grd", 
                                 ...
                                 discrete_breaks = seq(0,10000, length.out = 11)) +
  theme(panel.border = element_blank(), legend.background = element_blank(), axis.text = element_blank(), legend.text = element_text(size = 10),plot.title = element_text(hjust = 0.5)) +
  ggtitle(paste0("isoGSM, GM: ",round(simpleawmean(isot_lyr_isoGSM, lats = seq(from = -90,to = 90,length.out = 94)), digits = 1), "mm/year"))
  ...
  plot_diff <- STACYmap_discrete(gridlyr =  Plot_lyr, 
                                 legend_names = list(grid = "P range (mm/year)"), graticules = T, 
                                 colorscheme = RColorBrewer::brewer.pal(9, 'Reds'), 
                                 limits_color = list("max" = 7000, "min" = 0),
                                 labels_discrete = c(0,"","","",2000,"","","",4000,"","","",6000,"",""), 
                                 discrete_breaks = seq(0,7000,by = 500)) +
...
plot <- ggarrange(plot_ECHAM5, plot_GISS, plot_CESM, plot_HadCM3, plot_isoGSM, plot_diff,
  ...
  plot  %>% ggsave(filename = "Plots/Supplement/SFig_4_SimOverview_PREC.pdf", plot = .,
                 width = 3*12, height = 2*8.3, units = 'cm', dpi = 'print', device = "pdf")
  plot  %>% ggsave(filename = "Plots/Supplement/SFig_4_SimOverview_PREC.png", plot = ., 
